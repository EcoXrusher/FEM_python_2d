    ...
    for el in grid.elements:
        ...
        if node0.boundary == True:
            if node1.boundary == True:
                ...
                if 3 == main.PUNKTY_CALKOWANIA:
                    print(uni.boundries)
                    tmp[0] = uni.boundries[0] * weight[0]
                    tmp[1] = uni.boundries[2] * weight[0]
                    #mid
                    tmp[4] = uni.boundries[1] * weight[1]
                    tmp[5] = uni.boundries[1] * weight[1]
                    #top
                    tmp[8] = uni.boundries[2] * weight[0]
                    tmp[9] = uni.boundries[0] * weight[0]
            ...
            if node3.boundary == True:
                ...
                if 3 == main.PUNKTY_CALKOWANIA:
                    print(uni.boundries)
                    tmp[0] = uni.boundries[0] * weight[0]
                    tmp[3] = uni.boundries[2] * weight[0]
                    #mid
                    tmp[4] = uni.boundries[1] * weight[1]
                    tmp[7] = uni.boundries[1] * weight[1]
                    #top
                    tmp[8] = uni.boundries[2] * weight[0]
                    tmp[11] = uni.boundries[0] * weight[0]
                el.hbc[3] = list(tmp)
                wall3 = list(tmp)
                # print(f'if 2 :{tmp}')
                tmp = list()
        if node2.boundary == True:
            if node3.boundary == True:
                ...
                if 3 == main.PUNKTY_CALKOWANIA:
                    print(uni.boundries)
                    tmp[2] = uni.boundries[0] * weight[0]
                    tmp[3] = uni.boundries[2] * weight[0]
                    #mid
                    tmp[6] = uni.boundries[1] * weight[1]
                    tmp[7] = uni.boundries[1] * weight[1]
                    #top
                    tmp[10] = uni.boundries[2] * weight[0]
                    tmp[11] = uni.boundries[0] * weight[0]
                el.hbc[2] = list(tmp)
                wall2 = list(tmp)
                # print(f'if 3 :{tmp}')
            ...
            if node1.boundary == True:
                ...
                if 3 == main.PUNKTY_CALKOWANIA:
                    print(uni.boundries)
                    tmp[1] = uni.boundries[0] * weight[0]
                    tmp[2] = uni.boundries[2] * weight[0]
                    #mid
                    tmp[5] = uni.boundries[1] * weight[1]
                    tmp[6] = uni.boundries[1] * weight[1]
                    #top
                    tmp[9] = uni.boundries[2] * weight[0]
                    tmp[10] = uni.boundries[0] * weight[0]
                el.hbc[1] = list(tmp)
                wall1 = list(tmp)
                # print(f'if 4 :{tmp}')
                tmp = list()
        ...
        #SECTION Vector P
            ...
            for i in range(4):
                for j in range(4):
                    ...
                    grid.matrix_h[(tmp[i] * grid.nN) + tmp[j]] += el.hbc[(i * 4) + j]
